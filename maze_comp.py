...
def test_size(start_size, end_size, iterations, heuristic, jump):
    ...
    filename = str(start_size) + "-" + str(end_size) + "_" + str(iterations) + "_" + "_" + str(heuristic) + ".csv"
        ...
        for size in tqdm(range(start_size, end_size + 1, jump)):
            ...
            for _ in range(iterations):
                ...
                avg_visited_tiles += round(
                    100 * (data[2] + data[3] + data[4]) / (data[0] + data[2] + data[3] + data[4]), 2)
...
def make_time_plot(start_size, end_size, iterations):
    folder_name = 'plots'
    create_folder(folder_name)
    file_name = str(start_size) + "-" + str(end_size) + "_" + str(iterations) + ".jpg"
    ...
    for heuristic in range(3):
        ...
        for i in range(2, 5):
            ...
            axs[heuristic][i - 2].set_ylabel('average ' + column_names[i], rotation=90)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 4:
        analysis_average_data(int(sys.argv[1]), int(sys.argv[2]), int(sys.argv[3]), int(sys.argv[4]))
    elif len(sys.argv) == 4:
        analysis_average_data(int(sys.argv[1]), int(sys.argv[2]), int(sys.argv[3]))
    elif len(sys.argv) == 3:
